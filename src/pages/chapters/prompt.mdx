...
export const promptCode = `async function getSystemPrompt(
  tools: Tools,
  model: string,
  ...
  return resolveSystemPromptSections([
    ...
    // ... tool prompts, skill prompts, agent instructions
    ...dynamicSections,
  ])
}`;

export const promptAnnotations = [
  {
    startLine: 1, endLine: 6,
    title: '函数签名', titleEn: 'Function Signature',
    body: 'getSystemPrompt() 接收工具列表、模型名称、工作目录和 MCP 客户端作为参数。返回 Promise<string[]>——一个字符串数组，每个元素是系统提示词的一个"段落"，最终会被拼接成完整的 System Prompt。',
    bodyEn: 'getSystemPrompt() takes tools, model name, working directories, and MCP clients as parameters. Returns Promise<string[]> — an array of strings, each a "section" of the system prompt, eventually concatenated into the full System Prompt.',
    color: '#3b82f6',
  },
  {
    startLine: 7, endLine: 14,
    title: '并行预加载', titleEn: 'Parallel Preloading',
    body: '使用 Promise.all() 并行加载技能命令列表、输出样式配置和环境信息。这些数据来自不同的源（文件系统、配置、运行时），并行获取避免串行等待。',
    bodyEn: 'Uses Promise.all() to load skill commands, output style config, and environment info in parallel. These come from different sources (filesystem, config, runtime), and parallel fetching avoids serial waiting.',
    color: '#22c55e',
  },
  {
    startLine: 19, endLine: 40,
    title: '动态段落注册', titleEn: 'Dynamic Section Registration',
    body: 'systemPromptSection() 是一个注册器，将每个段落包装为惰性求值的函数。段落包括：会话指导、记忆文件、环境信息、语言偏好、输出样式、MCP 指令、暂存区等。DANGEROUS_uncachedSystemPromptSection 标记不可缓存的段落——因为 MCP 服务器可能在对话中途连接/断开。',
    bodyEn: 'systemPromptSection() is a registrar that wraps each section as a lazily-evaluated function. Sections include: session guidance, memory, env info, language, output style, MCP instructions, scratchpad, etc. DANGEROUS_uncachedSystemPromptSection marks uncacheable sections — because MCP servers can connect/disconnect mid-conversation.',
    color: '#f59e0b',
  },
  {
    startLine: 42, endLine: 43,
    title: '工具结果压缩指令', titleEn: 'Tool Result Summarization',
    body: 'SUMMARIZE_TOOL_RESULTS_SECTION 指示模型在某些情况下压缩工具返回的结果，减少上下文占用。getFunctionResultClearingSection 则根据模型能力决定是否启用"函数结果清理"优化。',
    bodyEn: 'SUMMARIZE_TOOL_RESULTS_SECTION instructs the model to summarize tool results in certain cases, reducing context usage. getFunctionResultClearingSection decides based on model capabilities whether to enable "function result clearing" optimization.',
    color: '#a855f7',
  },
  {
    startLine: 47, endLine: 53,
    title: '最终组装', titleEn: 'Final Assembly',
    body: 'resolveSystemPromptSections() 将所有段落解析并拼接。注意顺序：先是身份声明（intro）、系统规则（system）、任务指南（doing tasks），然后是工具提示词、技能指令、Agent 指令，最后是动态段落。顺序很重要——它影响模型对不同指令的注意力权重。',
    bodyEn: 'resolveSystemPromptSections() resolves and concatenates all sections. Note the order: identity (intro), system rules, task guidelines first, then tool prompts, skill instructions, agent instructions, and finally dynamic sections. Order matters — it affects the model\'s attention weight on different instructions.',
    color: '#ec4899',
  },
];
...
Claude Code 的 System Prompt 是一段大约 **12,000 tokens** 的长文本，在每次请求时都会被注入。实际上，它并不是一个硬编码的字符串，而是由多个源文件动态组装而成的——核心指令、工具提示词（每个工具都有自己的 <code>.description()</code> 方法）、技能指令、Agent 指令、CLAUDE.md、记忆文件等部分依次拼接。它是我行为的基因编码，定义了：
...
<p>不再是伪代码——这是 Claude Code 中 System Prompt 组装逻辑的<strong>真实源码</strong>（精简版）。点击高亮行或右侧批注，阅读每段代码的作用：</p>
...
Claude Code's system prompt is a text block of roughly **12,000 tokens**, injected with every request. In reality, it is not a single hardcoded string -- it is dynamically assembled from multiple source files: core instructions, tool prompts (each tool has its own <code>.description()</code> method), skill instructions, agent instructions, CLAUDE.md, and memory files, all concatenated in order. It is the genetic code that governs my behavior:
...
<p>No more pseudocode -- this is the <strong>actual source code</strong> of Claude Code's system prompt assembly (simplified). Click highlighted lines or sidebar annotations to read what each section does:</p>
